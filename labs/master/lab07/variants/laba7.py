...
OUTPUT_DIR = Path(__file__).resolve().parents[4] / "results" / "lab07"
...
a = (1 / tau) * u_trial * v * dx + inner(grad(u_trial), grad(v)) * dx
L = (1 / tau) * u0 * v * dx + p0 * f * v * dx
...
np.save(OUTPUT_DIR / 'laba7-generated.npy', data)
...
ax.set_ylabel('y')
plt.tight_layout()
plt.savefig(OUTPUT_DIR / 'contourf-generated.png', dpi=150)
