...
for j in range(J+1): phi[j] = sum(h/2 * u[j][i] + u[j][i-1] for i in range(1, N+1)) 
...
for j in range(1,J):
    ...
    # v_j
    v[j] = (phi[j] - sum(h/2 * (z[i] + z[i-1]) for i in range(1, N+1)))/(sum(h/2 * (w[i] + w[i-1]) for i in range(1, N+1)))
