...
Использование:
  python gif_to_png.py solution.gif            # первый кадр → solution.png
  python gif_to_png.py solution.gif --all      # все кадры → solution_000.png, ...
  python gif_to_png.py *.gif --all             # несколько GIF сразу
  python gif_to_png.py solution.gif -o out/    # указать папку вывода
    ...
    with Image.open(gif_path) as img:
        ...
        if not all_frames or n_frames == 1:
            ...
            for i in range(n_frames):
                img.seek(i)
                frame = img.convert("RGBA")
                out_name = f"frame0{str(i).zfill(digits)}.png"#f"{gif_path.stem}_{str(i).zfill(digits)}.png"
